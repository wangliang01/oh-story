...
**OpenClaw users:** Current support is skills-only. OpenClaw can discover the 22 story skills from workspace `skills/`, `.agents/skills`, `~/.agents/skills`, `~/.openclaw/skills`, or configured extra skill roots. `SKILL.md` files use OpenClaw-compatible single-line `name` / `description` plus single-line JSON `metadata.openclaw`. When `story-setup` targets OpenClaw, it copies the skills into project `skills/` and writes an OpenClaw `AGENTS.md`; agents/hooks are intentionally deferred, so outline-before-prose guards are soft skill checks rather than runtime enforcement. If new skills do not appear immediately, open a fresh OpenClaw session or wait for the skills watcher to refresh.
...
| `story-conception` | `/story-conception` | Conception router — interview-driven story-bible.md (premise → characters → plot → world → review) |
| `story-premise` | `/story-premise` | Premise / audience / tone — one-line pitch formula + platform specifics |
| `story-platform` | `/story-platform` | Platform × length rule selector — Yanxuan/Fanqie format & rhythm baselines + market signals |
| `story-world` | `/story-world` | Worldbuilding — three questions, power-system consistency, detail lists |
| `story-character` | `/story-character` | Character design — desire/flaw/trump, three support archetypes, villain logic |
| `story-plot` | `/story-plot` | Plot design — three-layer structure, conflict formula, pacing, emotional curve |
| `story-qualify` | `/story-qualify` | Signing-grade validation — 5 parallel sub-agents + signing verdict + calibration loop |
| `story-validate` | `/story-validate` | Review & rescue — golden-template 8 steps + eight pitfalls self-check, broken-story rescue |
| `story-learn` | `/story-learn` | Corpus self-learning — samples in, baselines updated, learning report out |
